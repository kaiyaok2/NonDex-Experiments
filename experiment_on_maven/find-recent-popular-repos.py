...
if __name__ == '__main__':
    ...
    for page in range(int(pages)):
        r = session.get("https://api.github.com/search/repositories?q=mvn+in:readme+" + filter + "&per_page=100&sort=stars&order=desc&page=" + str(page + 1))
        #r = session.get("https://api.github.com/search/repositories?q=mvn+in:readme+stars:200..30000+&per_page=100&sort=stars&order=desc&page=" + str(page + 1))
        r = r.json()
        for item in r['items']:
